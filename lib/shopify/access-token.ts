...
async function requestAccessToken(): Promise<string> {
  ...
  if (response.status === 400 || response.status === 401 || response.status === 403) throw new ShopifyAuthenticationError();
  if (!response.ok) throw new ShopifyNetworkError();
  let payload: unknown;
  try { payload = await response.json(); } catch { throw new ShopifyAuthenticationError(); }
  if (!isTokenResponse(payload)) throw new ShopifyAuthenticationError();
