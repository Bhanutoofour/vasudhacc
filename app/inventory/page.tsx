...
export default async function InventoryPage() {
  ...
  return <div className="space-y-6"><div><p className="text-xs font-medium text-[#2d725f]">Inventory</p><h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900">{feed.mode === "snapshot" ? "Inventory intelligence" : "Live inventory intelligence"}</h1><p className="mt-1 text-sm text-slate-500">{feed.mode === "snapshot" ? "Compare stock across three daily snapshots and locations." : "Inspect the current Shopify inventory while history snapshots warm up."}</p></div><SnapshotStatusStrip feed={feed}/><InventoryTable items={feed.items} feed={feed}/></div>;
}
