...
export default async function Home() {
  ...
  return <div className="space-y-7">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-medium text-[#2d725f]">Inventory overview</p><h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900">Good morning, Vasudha</h1><p className="mt-1 text-sm text-slate-500">{feed.mode === "snapshot" ? "Here is how your inventory is performing across the latest snapshots." : feed.mode === "current" ? "Here is the live Shopify inventory while snapshots are still warming up." : "Here is the current inventory view."}</p></div></div>
